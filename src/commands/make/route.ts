...
export default class MakeRoute extends Command {
  static description =
    "Make route will create all folder and files you need to work";
  ...
  inquirer
    ...
    .then((answer) => {
      fs.readFile("./package.json", "utf8", (err, data) => {
        ...
        const exts = ["html", "ejs", "edge", "pug"];

        if (!JSON.parse(data).cli || !JSON.parse(data).cli.engine) {
          ux.error(
            `Dans votre package.json veuillez spécifier le moteur de template : \n\t${CustomColors.cmd(
              '"cli": {\n\t  "engine": "html"\n\t}\n\n'
            )}`,
            { code: "400" }
          );
        ...
        ext = JSON.parse(data).cli.engine.toLowerCase();

        if (!exts.find((el) => el === ext)) {
          ux.error(`.${ext} n'est pas une extension pris en charge`, {
            code: "400",
          });
        }
      });

      if (!answer.route.includes("/"))
        answer.route = `${answer.route}/${answer.route}`;
      ...
      if (view.includes("/") || view.includes("\\")) {
        ...
        fs.open(`./routers/${folder}/${view}.js`, (err) => {
          if (!err) {
            throw new Error(
              `le ficher ${process.cwd()}/routers/${folder}/${view}.js existe deja !`
            );
          }
          ...
          fs.open(`./resources/css/${folder}/${view}.css`, (err) => {
            if (!err) {
              throw new Error(
                `le ficher ${process.cwd()}/resources/css/${folder}/${view}.css existe deja !`
              );
            }
            ...
            fs.open(`./resources/js/${folder}/${view}.js`, (err) => {
              if (!err) {
                throw new Error(
                  `le ficher ${process.cwd()}/resources/css/${folder}/${view}.js existe deja !`
                );
              }
              ...
              fs.open(`./resources/views/${folder}/${view}.${ext}`, (err) => {
                if (!err) {
                  throw new Error(
                    `le ficher ${process.cwd()}/resources/views/${folder}/${view}.${ext} existe deja !`
                  );
                } else {
                  const routeWrite = `const { Router } = require('express');\nconst router = Router();\n\nrouter.get('/${
                    ...
                    ext === "html"
                      ? `res.sendFile(\`\${process.cwd()}\}/${answer.route}\`)`
                  ...
                  fs.writeFileSync(
                    `./routers/${folder}/${view}.js`,
                    routeWrite
                  );
                  fs.writeFileSync(
                    `./resources/views/${folder}/${view}.${ext}`,
                    viewWrite
                  );
                  fs.writeFileSync(
                    `./resources/css/${folder}/${view}.css`,
                    cssWrite
                  );
